...
public class GameSessionChannelListener : IDisposable
{
    ...
    private void OnReceiveReady(object? sender, NetMQSocketEventArgs e)
    {
        ...
        try
        {
            ...
            if (fullPayload.Length < GSCHeader.Size)
            {
                _logger.LogWarning("Invalid GameSessionChannel packet (payload too small)");
                return;
            }

            var header = MemoryMarshal.Read<GSCHeader>(fullPayload);
            ...
            var clientData = fullPayload.Slice(GSCHeader.Size);

            // Client 패킷은 [EndPointHeader][GameHeader][Payload] 구조이므로 EndPointHeader를 건너뜀
            if (clientData.Length < EndPointHeader.Size + GameHeader.Size)
            {
                _logger.LogWarning("Invalid payload size from Gateway");
                return;
            }

            var gameHeader = MemoryMarshal.Read<GameHeader>(clientData.Slice(EndPointHeader.Size));
            ...
            if (activity != null)
            {
                ...
                activity.SetTag("packet.size", fullPayload.Length - GSCHeader.Size);
            }
            ...
            context.TransferPayload(ref payloadMsg, GSCHeader.Size);
    ...
    /// </summary>
    public void SendResponse(long gatewayNodeId, long sessionId, Response response, ushort requestId)
    {
        try
        {
            if (response.IsError)
            {
                SendErrorResponse(gatewayNodeId, sessionId, (short)response.ErrorCode, requestId);
                return;
            }
            ...
            var totalSize = GSCHeader.Size + EndPointHeader.Size + GameHeader.Size + payloadSize;
            ...
            {
                TotalLength = EndPointHeader.Size + GameHeader.Size + payloadSize,
                Flags = flags
            };
    ...
    /// </summary>
    private void SendErrorResponse(long gatewayNodeId, long sessionId, short errorCode, ushort requestId)
    {
        var totalSize = GSCHeader.Size + EndPointHeader.Size + GameHeader.Size;
        var msg = new Msg();
        msg.InitPool(totalSize);
        ...
        var gameHeader = new GameHeader { RequestId = requestId };
        gameHeader.Write(span[offset..]);
