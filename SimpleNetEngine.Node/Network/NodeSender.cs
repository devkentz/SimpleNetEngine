    ...
    : INodeSender
{
        ...
        where TResponse : IMessage<TResponse>, new()
    {
        ...
        // Distributed Tracing: Client span (full round-trip)
        var msgName = AutoGeneratedParsers.GetNameById(packet.Header.MsgId);
        using var activity = TelemetryHelper.Source.StartActivity($"Node.Send {msgName}", ActivityKind.Client);
        activity?.SetTag("rpc.method", msgName);
        activity?.SetTag("rpc.service", $"Node:{targetNode.Identity}");
